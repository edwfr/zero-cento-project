...
**Note:** La rimozione delle righe esercizio in edit programma trainer ora avviene in modo immediato al drop sul cestino: per le righe persistite parte subito la `DELETE` API (con `404` trattato in modo idempotente), mentre le righe draft vengono rimosse localmente. Rimossa tutta la logica deferred basata su `pendingDeletesByWorkout` e relativo codice di cleanup nel salvataggio workout. Confermata inoltre la rimozione del delete in riga via conferma/modal: resta solo il flusso drag-and-drop sul cestino.
