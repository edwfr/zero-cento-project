...
**Note:** I tre gruppi sotto soglia passano l'obiettivo dell'80% su tutte e quattro le metriche: `src/schemas/**` da 79.5/79.6/80.8/68.2 a **95.5/95.5/100/87.6**, `src/lib/**` da 76/75/75/73 a **94.8/94.6/97.8/89.0**, `src/app/api/**` da 57.9/56.9/64.1/54.2 a **92.1/91.6/95.7/87.1**; il totale su `src/` sale da 38/37/30/35 a **47.2/46.9/35.9/43.4**. Suite da 993 a **1324 test verdi**. Coperti da zero `program-status.ts`, l'export PDF del programma, `sync-user-metadata`, il provider i18n e, sul fronte API, gli esercizi di un workout, i report di programma, `review`/`complete`/`copy-first-week`, le rotte di autenticazione, `weeks/[id]`, `health`, i dati di riferimento (dettaglio, archiviazione, colori dei pattern), attivazione e disattivazione atleta, `users/[id]` (GET e PUT erano scoperti del tutto), le azioni admin (override programma e riassegnazione atleta), le note dell'atleta sul workout, il recap del workout, `PUT /api/programs/[id]` con l'allineamento di settimane e sedute, il report delle serie pianificate e gli error path di `exercises/[id]`. Ogni rotta è verificata su 200/201, 400, 401, 403 di ruolo, 403 di appartenenza e 404, con `toHaveBeenCalledWith` su ogni scrittura Prisma. Scrivendo i test sono emersi due bug veri, corretti a parte: `trainee-program-data.ts` riportava anche le serie dei feedback vecchi (ora tiene solo l'ultimo feedback per esercizio) e `POST .../workouts/[workoutId]/exercises` aveva un ramo irraggiungibile nel calcolo dell'ordine. Soglie alzate ai valori misurati — globale 40/39/32/36 → **47/46/35/43**, `src/app/api/**` 57/56/63/54 → **92/91/95/87** — e restano regola: salgono, non scendono mai.
### [20 Settembre 2026] — Gruppi muscolari opzionali sugli esercizi (0-n, nessun cap)

**File modificati:** `src/schemas/exercise.ts`, `src/app/api/exercises/route.ts`, `src/app/api/exercises/[id]/route.ts`, `src/app/trainer/exercises/new/_content.tsx`, `src/app/trainer/exercises/[id]/edit/_content.tsx`, `src/app/trainer/exercises/_content.tsx`, `src/components/ExerciseCreateModal.tsx`, `src/components/ExercisesTable.tsx`, `public/locales/{en,it}/{trainer,validation}.json`, `tests/unit/schemas.test.ts`, `tests/unit/lib/api-error.test.ts`, `tests/integration/exercises.test.ts`, `implementation-docs/CHANGELOG.md`
**Note:** Un esercizio può ora essere associato a 0-n gruppi muscolari, senza limite superiore. Prima `exerciseSchema` imponeva `.min(1)` e `.max(5)`, e i due handler API rifiutavano con 400 una somma dei coefficienti fuori da 0.1-3.0: un trainer non poteva salvare un esercizio senza incidenza muscolare né distribuirla su più di 5 gruppi. Il campo diventa `z.array(...).default([])`, così accetta l'array vuoto e anche l'omissione del campo; il cap sulla somma sparisce da `POST /api/exercises`, da `PUT /api/exercises/[id]` e dalla validazione client dei due form, e il bottone "Aggiungi" di `ExerciseCreateModal` non si disabilita più a 5 voci. Resta il `coefficient` per singolo gruppo tra 0 e 1 (è la percentuale d'incidenza) e resta la verifica che ogni `muscleGroupId` esista, saltata quando l'array è vuoto. Nessuna migrazione: `ExerciseMuscleGroup` è già una relazione separata e il vincolo `@@unique([exerciseId, muscleGroupId])` continua a impedire i duplicati. I consumer di report e grafici muscolari iterano la relazione con `forEach`, quindi un esercizio senza gruppi contribuisce zero volume senza modifiche. Nelle liste esercizi (trainer e `ExercisesTable`) la cella vuota mostra ora un trattino. Chiavi i18n rimosse perché orfane: `validation.minOneMuscleGroup`, `validation.maxMuscleGroups`, `exercises.coefficientRangeError`; le label dei form non annunciano più "min 1, max 5" né i totali `/ 1.0` e `/ 3.0`.
### [20 Settembre 2026] — Test quality, fase 2: auth.ts e middleware.ts testati davvero

**File modificati:** `src/lib/date-format.ts`, `vitest.config.ts`, `tests/unit/date-format.test.ts`, `tests/unit/lib/auth.test.ts` (nuovo), `tests/unit/middleware.test.ts` (nuovo, assorbe `tests/unit/middleware-must-change-password.test.ts`), `tests/integration/auth-session.test.ts`, `tests/integration/rbac.test.ts`, `.claude/skills/zero-cento-testing/SKILL.md`, `implementation-docs/CHANGELOG.md`
**Note:** `src/lib/auth.ts` passa da L22.8/B31.8 a **100% di righe e rami** e `src/middleware.ts` da L49.3 a **L90.7/B92.2**: i due file che decidono chi può fare cosa erano quelli meno coperti, perché ogni test li mockava invece di eseguirli. I nuovi test mockano solo il confine — `@/lib/supabase-server`, `@supabase/ssr`, `@upstash/redis/cloudflare` e Prisma — e lasciano girare l'implementazione vera; `cache` di React diventa una funzione passthrough, altrimenti il modulo non è caricabile sotto jsdom. Incluse due regressioni di sicurezza sull'hotfix `app_metadata`: un `role` scritto dall'utente in `user_metadata` non conferisce privilegi, né sul percorso veloce né sul fallback Prisma. Corretto un bug vero in `getTodayForInput`, che usava `toISOString()` e quindi il giorno **UTC**: alle 23:30 in Italia restituiva il giorno precedente, datando un giorno indietro i record creati a tarda sera; ora costruisce la data locale, con test a timer finti sul fuso `Europe/Rome`. La suite gira con `TZ=UTC` fisso (`vitest.config.ts`), così non dipende più dal fuso della macchina. Riparato anche `tests/integration/auth-session.test.ts`, che il merge della fase 1 con l'hotfix aveva lasciato rotto (riferimento a `prisma` invece di `prismaMock` e due `as any`): erano 1 test rosso, 1 errore di type-check e 2 di lint su `development`. `rbac.test.ts` resta test di collegamento e ora asserisce gli argomenti delle guardie, non solo lo status. Soglie alzate: globale 36/36/28/33 → 38/37/30/35, `src/lib/**` 67/67/65/62 → 76/75/75/73. Suite: 993 test verdi.
### [20 Settembre 2026] — Test quality, fase 1: mock condivisi e pulizia dei test

**File modificati:** `tests/helpers/prisma-mock.ts`, `tests/helpers/sessions.ts`, `tests/helpers/auth-mock.ts`, `tests/helpers/auth-module-mock.ts`, `tests/helpers/call-args.ts`, `tests/unit/setup.ts`, tutti i 30 file in `tests/integration/` (eliminato `tests/integration/fixtures.ts`), `tests/unit/{completion-service,NewProgramContent,transform-utils,trainee-workout-focus,trainer-trainee-programs-tab,trainer-trainee-detail-sbd-report}.test.tsx?`, `tests/unit/lib/{trainee-program-data,api-error}.test.ts`, `tests/unit/components/ErrorBoundary.test.tsx`, `eslint.config.mjs`, `package.json`, `.claude/skills/zero-cento-testing/SKILL.md`, `implementation-docs/CHANGELOG.md`
**Note:** 29 file di integrazione ridichiaravano a mano il mock di Prisma e ~190 `as any` impedivano ai tipi di segnalare mock divergenti dallo schema. Ora `tests/helpers/prisma-mock.ts` espone un `mockDeep<PrismaClient>()` agganciato una volta sola nel setup globale e azzerato prima di ogni test, con `$transaction` in entrambe le forme (callback interattivo eseguito contro lo stesso mock, array risolto in ordine): i client di transazione costruiti a mano nei test spariscono. Le sessioni passano a `tests/helpers/sessions.ts` con un vero `AuthSession` (Supabase user tipizzato) e le guardie si configurano con `asTrainer`/`asAdmin`/`asTrainee`/`asUnauthenticated`/`asForbidden`. L'automock `vi.mock('@/lib/auth')` previsto dal piano non è utilizzabile — valuta il modulo vero, che in cima chiama `cache` di React, assente nel build client di React 19 — quindi i file usano la fabbrica condivisa `authModuleMock()`. Tutti gli `as any` sono spariti dai test (unico varco: `as never` sull'argomento di `mockResolvedValue`, più `callArg()` per rileggere una chiamata registrata), così come i `require()`. ESLint ora gira anche su `tests/` con `no-explicit-any` e il divieto di `require()`. Un test di `rbac.test.ts` passava grazie a uno stub lasciato dal test precedente (`clearAllMocks` non azzera le implementazioni, `mockReset` sì): ora lo configura esplicitamente. Stesso numero di test prima e dopo: 331 nei file di integrazione.
### [20 Settembre 2026] — Test quality, fase 0: suite e CI verdi

**File modificati:** `tests/unit/setup.ts`, `tests/unit/setup-storage.test.ts`, `tests/unit/workout-recap-panel.test.tsx`, `tests/unit/records-exercise-autocomplete.test.tsx`, `tests/unit/trainee-workout-focus.test.tsx`, `src/app/trainer/trainees/[id]/records/_content.tsx`, `vitest.config.ts`, `.claude/skills/zero-cento-testing/SKILL.md`, `implementation-docs/CHANGELOG.md`
**Note:** La suite falliva in locale su Node ≥25 (31 test) e la CI non rispettava la soglia di coverage dell'80% sulle branch (76.2%). `tests/unit/setup.ts` installa ora un `localStorage`/`sessionStorage` in memoria quando quello dell'ambiente non è utilizzabile (su Node ≥25 il webstorage nativo copre quello di jsdom e lancia un errore senza `--localstorage-file`), azzerati prima di ogni test: niente più `NODE_OPTIONS` negli script. `workout-recap-panel.test.tsx` allinea le asserzioni al formato attuale di `buildExerciseSpec` (`3 x 5 · 120kg · @RPE 8`) e copre i rami senza peso e senza RPE. `records-exercise-autocomplete.test.tsx` e `trainee-workout-focus.test.tsx` sostituiscono il barrel `@/components` con un mock (l'import trascinava MUI e recharts, ~48s sotto jsdom, con timeout dei test), usano import statici e un `fetch` instradato per URL; la query del bottone Next è ristretta alla bottom-nav con il nome esatto `workouts.next`. La pagina dei massimali importa i tre componenti dai rispettivi file invece che dal barrel. `vitest.config.ts` misura la coverage su tutto `src/` (al posto della lista a mano di 37 file) con esclusioni mirate, soglie a due livelli — globale L36/S36/F28/B33 più i gruppi `src/lib/**`, `src/schemas/**`, `src/app/api/**` — e `maxWorkers: 4` per tenere in piedi il fork pool su WSL. Le soglie sono la baseline misurata e vanno solo alzate. Suite: 864 test verdi.
### [20 Settembre 2026] — Hotfix sicurezza: autorizzazione da `app_metadata`

**File modificati:** `src/lib/sync-user-metadata.ts`, `src/lib/auth.ts`, `src/middleware.ts`, `src/app/api/auth/force-change-password/route.ts`, `src/app/api/users/route.ts`, `src/app/login/page.tsx`, `src/app/onboarding/set-password/page.tsx`, `scripts/migrate-auth-metadata.ts`, `package.json`, `tests/unit/lib/sync-user-metadata.test.ts`, `tests/unit/lib/auth-metadata.test.ts`, `tests/unit/middleware-must-change-password.test.ts`, `tests/integration/auth-session.test.ts`, `tests/integration/users.test.ts`, `implementation-docs/CHANGELOG.md`
**Note:** `getSession()` leggeva `role` e `isActive` dai `user_metadata` di Supabase sul fast path, saltando Prisma. I `user_metadata` sono scrivibili dall'utente autenticato con la chiave anon (`supabase.auth.updateUser({ data })`), quindi un trainee poteva assegnarsi `role: 'admin'` dalla console del browser e accedere a tutte le rotte admin, e un utente disattivato poteva rimettersi `isActive: true`; stessa origine per `mustChangePassword` (`middleware.ts`, pagina di login), che l'utente poteva azzerare da solo saltando il cambio password forzato. I dati di autorizzazione (`role`, `isActive`, `mustChangePassword`) passano ora in `app_metadata`, scrivibile solo dalla service role tramite `syncUserMetadata`; `firstName`/`lastName` restano in `user_metadata` perché sono solo di visualizzazione. `getSession()` e `getSessionIncludingInactive()` leggono l'autorizzazione da `app_metadata` e mantengono il fallback su Prisma (fonte di verità) per gli utenti non ancora migrati, così nessuna sessione esistente si rompe durante il rilascio. L'invito utente ora scrive il ruolo in `app_metadata` (`inviteUserByEmail` riempie solo `user_metadata`); il redirect di fine onboarding usa il fallback `/api/auth/me` invece di costruire `/undefined/dashboard` quando il ruolo manca. Aggiunti test di regressione: un `role` o un `isActive` iniettati in `user_metadata` non hanno effetto. **Rilascio:** eseguire `npm run migrate:auth-metadata` (prima con `--dry-run`) sul progetto Supabase di produzione **prima** del deploy; lo script segnala anche gli utenti i cui `user_metadata` divergono dal database, cioè i possibili tentativi di escalation. I vecchi campi in `user_metadata` non vengono cancellati: il codice nuovo li ignora e lasciarli consente il rollback.
...
**Note:** Il DELETE `/api/users/[id]` falliva per vincolo FK su qualsiasi atleta con dati, perché `TrainingProgram.trainee`, `ExerciseFeedback.trainee` e `PersonalRecord.trainee` non hanno `onDelete: Cascade`, e non rimuoveva l'account Supabase Auth. Inoltre usava `requireAuth()` controllando la ownership solo per i trainer, quindi un trainee poteva eliminare altri utenti non admin: ora usa `requireRole(['admin', 'trainer'])`. Per target trainee l'handler elimina prima l'utente Supabase (un 404 è trattato come successo, così un retry dopo un errore DB completa l'operazione; qualsiasi altro errore restituisce 500 `user.deleteFailed` senza toccare il DB), poi in una `prisma.$transaction` cancella programmi (cascade su weeks → workouts → workoutExercises → feedback → setsPerformed e skeletons), feedback, massimali e l'utente (cascade su `TrainerTrainee`). Delete esplicite invece di cascade in schema: nessuna migrazione e nessuna cancellazione a cascata accidentale da altri percorsi. Target non trainee invariati. In `/trainer/trainees` aggiunto il bottone elimina che apre `ConfirmationModal` (danger) con l'elenco dei dati cancellati; la modale admin esistente (`UserDeleteModal`) beneficia del fix e ha un avviso aggiornato. Aggiunto guard contro doppio click sulla conferma; errore DB dopo eliminazione Supabase ora restituisce `user.deleteFailed` (invita a riprovare) con log dedicato; messaggio di successo italiano neutro.
### [19 Settembre 2026] — Risultati workout: valori pianificati vs eseguiti e RPE per serie

**File modificati:** `src/app/api/programs/[id]/test-results/route.ts`, `src/app/trainer/programs/[id]/tests/_content.tsx`, `public/locales/en/trainer.json`, `public/locales/it/trainer.json`, `tests/integration/program-test-results.test.ts`, `tests/unit/program-test-results-content.test.tsx`, `implementation-docs/CHANGELOG.md`
**Note:** Nella tabella di `/trainer/programs/[id]/tests` le colonne Set/Ripetizioni/RPE/Peso mescolavano valori pianificati e valori del feedback (fallback dall'uno all'altro), e l'RPE dichiarato per ogni serie (`SetPerformed.actualRpe`) non veniva mostrato. L'API `GET /api/programs/[id]/test-results` ora restituisce i valori pianificati puri (`sets`, `reps`, `rpe` = `targetRpe`, nuovo `plannedWeight` = `effectiveWeight`) e i valori eseguiti per serie dall'ultimo feedback, uniti con ` / `: `repsDone`, `rpeDone` (nuovi) e `weightUsed`; `-` quando non c'è feedback, e `-` nella posizione di una serie senza RPE. L'RPE a livello feedback non è più usato. Colonne UI: Esercizio | Set | Rep | RPE | Peso | Rep. fatte | RPE percepito | Peso utilizzato | (i). La colonna Commenti è sostituita da un'icona info mostrata solo se l'esercizio ha un commento, con tooltip MUI (portal, per non essere tagliato dal wrapper `overflow-x-auto`; apertura immediata al tap su mobile). Nuove chiavi `colPlannedWeight`, `colRepsDone`, `colRpeDone`, `colCommentInfo`; rimosse `colComments` e `noComments` rimaste orfane.
### [18 Settembre 2026] — Eliminazione programmi consentita in qualsiasi stato

**File modificati:** `src/app/api/programs/[id]/route.ts`, `src/components/ProgramTraineeTable.tsx`, `src/components/ConfirmationModal.tsx`, `src/app/trainer/programs/_content.tsx`, `src/app/trainer/trainees/[id]/_content.tsx`, `public/locales/en/trainer.json`, `public/locales/it/trainer.json`, `public/locales/en/errors.json`, `public/locales/it/errors.json`, `tests/integration/program-detail.test.ts`, `tests/unit/trainer-programs-content.test.tsx`, `tests/unit/trainer-trainee-programs-tab.test.tsx`, `implementation-docs/CHANGELOG.md`
**Note:** Il DELETE `/api/programs/[id]` consentiva al trainer di eliminare solo i programmi in bozza (403 `program.cannotDeleteNonDraft` sugli altri), mentre l'admin poteva già eliminare qualsiasi programma. Rimosso il guard sullo stato: resta il solo check di ownership (il trainer elimina solo i programmi di cui è proprietario). L'eliminazione si appoggia alle `onDelete: Cascade` già presenti nello schema, che rimuovono weeks → workouts → workoutExercises → exerciseFeedbacks → setsPerformed più i `workoutSkeletons`; i `personalRecords` sono legati a trainee ed esercizio, non al programma, e restano intatti di proposito. Lato UI il bottone elimina era renderizzato solo sulle righe in bozza di `ProgramTraineeTable`: ora compare anche sulle righe attive e completate, quindi su entrambe le schermate che usano la tabella (`/trainer/programs` e `/trainer/trainees/[id]`). La callback `onDeleteProgram` riceve anche lo `status` così i due handler possono aggiungere al messaggio di conferma la nuova chiave `programs.confirmDeleteProgramWarning` quando il programma non è in bozza, avvisando che feedback e serie completate andranno persi; `ConfirmationModal` rende il messaggio con `whitespace-pre-line` per mostrare l'avviso su una riga separata. Rimossa la chiave orfana `program.cannotDeleteNonDraft` da entrambi gli `errors.json`.
...
**Note:** Il pannello "Report SBD fondamentali" della tab Reportistica in `/trainer/trainees/[id]` calcolava FRQ/NBL/IM con formule diverse dalla schermata `/trainer/programs/[id]` (FRQ = numero di punti timeline invece dei workout distinti; IM sempre `null`). Ora entrambe le schermate usano le stesse funzioni pure di `src/lib/program-sbd-metrics.ts` come source of truth: FRQ = workout distinti per settimana, NBL = Σ `sets × reps` non-warmup, IM = media pesata per liftCount. Aggiunta priorita RPE nel calcolo IM: se `targetRpe` e presente sull'esercizio, il helper legge la % da `MIKE_TUCHSCHERER_RPE_CHART[plannedReps][targetRpe]` e la usa come intensita (override su `percentage_1rm` e `absolute`); nuovo export `intensityFromRpeChart(reps, rpe)` in `calculations.ts` e nuovo `aggregateWeekMetrics` in `program-sbd-metrics.ts` per l'aggregazione sulla finestra temporale (Σ FRQ, Σ NBL, IM media pesata per NBL). L'endpoint `GET /api/users/[id]/reports/planned-training-sets` seleziona ora anche `weightType/weight/targetRpe/exercise.id`, calcola `normalizedOneRMByExercise` dal PR-map del trainee e serializza `fundamentalMetrics` per punto tramite gli stessi helper condivisi (retrocompatibilita mantenuta su `fundamentalSets`/`fundamentalLifts`); il codice locale duplicato (`FUNDAMENTAL_PATTERNS`, `matchFundamentalLift` inline) e stato rimosso. Nel pannello trainee sostituito `getSbdPointMetric` (che restituiva IM null) con la lettura di `point.fundamentalMetrics[lift]`; le KPI card e la tabella per-colonna mostrano ora l'IM. Test coperti: RPE-priority (override), RPE su `absolute` senza 1RM, mix RPE + %1RM con media pesata, `aggregateWeekMetrics`, regressione FRQ = Σ frequency (2 workout squat/settimana → FRQ=2 nel programma e nel trainee). Bonus fix: alzato `z-index` dell'header sticky del `ProgramChartsDrawer` da `z-10` a `z-30` per evitare il bleed-through delle etichette sticky-left del heatmap muscle-group.
...
**Note:** Nella schermata `/trainer/programs/[id]/tests` i pannelli di settimana e workout sono ora espandibili/collassabili con comportamento coerente al pattern trainee: stato iniziale aperto, toggle indipendenti, preservazione dello stato durante il refresh dati e attributi `aria-expanded` per accessibilita. In ogni pannello settimana e stato aggiunto anche il badge tipo settimana (test, volume, deload, ecc.) tramite `WeekTypeBadge` con labels i18n `trainer:weekTypes.*`. Aggiunte chiavi i18n dedicate per le azioni di apertura/chiusura (EN/IT) e test unitario esteso per coprire toggle + badge.
...
**File modificati:** `src/lib/program-guards.ts` (NUOVO), `src/app/api/programs/[id]/route.ts`, `src/app/api/programs/[id]/copy-week/route.ts`, `src/app/api/programs/[id]/workouts/[workoutId]/exercises/bulk/route.ts`, `src/app/api/programs/[id]/workouts/[workoutId]/exercises/route.ts`, `src/app/api/programs/[id]/workouts/[workoutId]/exercises/[exerciseId]/route.ts`, `src/app/api/programs/[id]/workouts/[workoutId]/exercises/reorder/route.ts`, `src/app/trainer/programs/_content.tsx`, `src/app/trainer/programs/[id]/edit/_content.tsx`, `src/app/trainer/programs/[id]/edit/transform-utils.ts`, `public/locales/{it,en}/errors.json`, `public/locales/{it,en}/trainer.json`, `tests/integration/workout-exercises-bulk.test.ts`, `tests/integration/workout-exercise-delete.test.ts`, `tests/integration/copy-week.test.ts`, `tests/integration/programs.test.ts`, `tests/integration/rbac.test.ts`, `tests/unit/trainer-programs-content.test.tsx`

**Note:** Implementata la modifica di programmi già pubblicati (status=active) dalla schermata `/trainer/programs`. Le modifiche si applicano solo ai workout non ancora iniziati dal trainee; i workout con almeno un `SetPerformed.completed=true` vengono bloccati in sola lettura con badge visivo. Comportamento invariato per programmi in bozza e per l'admin (bypassa sempre i guard).
- **Backend guard `hasWorkoutStarted`**: helper `src/lib/program-guards.ts` che conta `setPerformed.count({ where: { completed: true, feedback: { workoutExercise: { workoutId } } } })`. Importato nei 4 handler di mutation esercizi (bulk PUT, POST singolo, PUT/DELETE singolo, PATCH reorder): logica trifase (completed → 403 cannotModifyNonDraft; active + started → 403 workoutStartedEditDenied; active + non-started → 200).
- **Copy-week guard**: il check `status !== 'draft'` è stato sostituito con: blocked se completed (stessa chiave); allowed se active + target week senza workout completati; blocked se active + target week con almeno 1 workout `isCompleted=true` con esercizi (chiave `program.copyWeekTargetProtected`).
- **GET /api/programs/[id]**: per sessioni trainer/admin su programmi active, aggiunta query `prisma.workout.findMany` per identificare i workout avviati e restituire `isStarted: boolean` per ogni workout nella risposta.
- **Frontend lista programmi**: aggiunto `ActionIconButton variant="edit"` per i programmi active nella tab attivi (`_content.tsx`).
- **Frontend editor (`edit/_content.tsx`)**: `isStarted?: boolean` aggiunto all'interfaccia `Workout`; auto-advance al passo 'details' forzato per programmi active/completed; pulsante "Modifica scheletro" e sezione skeleton nascosti su programmi non-draft; variabile `workoutIsLocked` per workout active+started disabilita l'action bar e tutti gli input di riga (via `rowBusy`) e il `SortableContext`; badge "Workout iniziato - non modificabile" visibile al posto dell'action bar.
- **transform-utils.ts**: mappa `isStarted: Boolean(workout.isStarted)` dal payload API.
- **i18n**: aggiunte chiavi `program.workoutStartedEditDenied`, `program.copyWeekTargetProtected` in errors.json EN/IT; `editProgram.workoutLockedStarted` in trainer.json EN/IT.
- **Test**: nuovi casi per bulk (active+started → 403, active+not-started → 200, completed → 403), delete (active+started → 403, active+not-started → 200), copy-week (target protetta → 403, active non-protetta → 200, completed → 403), trainer-programs-content (edit button visibile su active). Aggiornati test esistenti in `programs.test.ts` e `rbac.test.ts` per riflettere nuova semantica.


- **[25 Maggio 2026] Trainee workout: ripristino step all'apertura** — Quando un trainee esce dalla pagina `/trainee/workouts/[id]` (chiude la web app, switcha tra app, preme indietro) e poi riprende l'allenamento, viene riportato sull'esercizio dove si era fermato invece che sempre sul primo. Implementato persistendo `currentStep` nel payload localStorage esistente (`workout_${workoutId}_feedback`). Al caricamento, `fetchWorkout()` legge il valore salvato e lo usa come step iniziale (con validazione bounds). `saveLocalData()` ora include `currentStep` nel JSON e lo ha nei deps del `useCallback`, così ogni navigazione tra esercizi viene salvata automaticamente. Nessuna modifica al DB, nessuna nuova API. **Files modificati:** `src/app/trainee/workouts/[id]/_content.tsx`.
...
**File modificati:** `prisma/schema.prisma`, `prisma/migrations/20260513000000_set_performed_actual_rpe/migration.sql`, `src/schemas/feedback.ts`, `src/app/api/trainee/workout-exercises/[id]/feedback/route.ts`, `src/app/api/trainee/workouts/[id]/submit/route.ts`, `src/app/api/trainee/workouts/[id]/prev-week/route.ts`, `src/app/api/trainee/workouts/[id]/route.ts`, `src/lib/workout-recap.ts`, `src/app/trainee/workouts/[id]/_content.tsx`, `src/components/WorkoutExerciseDisplayList.tsx`, `src/components/PrevWeekPanel.tsx`, `public/locales/en/trainee.json`, `public/locales/it/trainee.json`, test unit/integration correlati.
**Note:** Implementato RPE opzionale per singola serie (5.0–10.0, step 0.5) con persistenza end-to-end. Aggiunta la colonna `actualRpe` su `sets_performed` e propagazione completa in schema Zod, autosave set (`PATCH /feedback`) e submit workout (`POST /submit`). Nella UI trainee focus mode la griglia serie ha ora una colonna `RPE` con selettore dedicato (`RPESelector`) editabile prima della spunta; in deselezione la riga viene resettata (reps/weight/RPE). Nel pannello settimana precedente le righe set mostrano `@ RPE x.x` quando disponibile; in assenza resta il formato precedente.

### [13 Maggio 2026] — Clone program trainer

**Task checklist:** #11.103
**File modificati:** `src/schemas/program.ts`, `src/app/api/programs/route.ts`, `src/components/ActionIconButton.tsx`, `src/app/trainer/programs/_content.tsx`, `src/app/trainer/trainees/[id]/_content.tsx`, `src/app/trainer/programs/new/page.tsx`, `src/app/trainer/programs/new/NewProgramContent.tsx`, `public/locales/en/trainer.json`, `public/locales/it/trainer.json`, `public/locales/en/errors.json`, `public/locales/it/errors.json`, `tests/unit/schemas.test.ts`, `tests/unit/NewProgramContent.test.tsx`, `tests/integration/programs-clone.test.ts`, `tests/integration/programs.test.ts`, `implementation-docs/CHECKLIST.md`.
**Note:** Aggiunta azione `Clone` su tutte le righe programma in `/trainer/programs` e nel tab programmi di `/trainer/trainees/[id]`, con navigazione verso `/trainer/programs/new?cloneFromProgramId=<id>` (preservando `traineeId` dal dettaglio atleta). La pagina server del wizard carica la sorgente clone con verifica ownership trainer/admin e passa `cloneSource` al client. `NewProgramContent` ora precompila atleta, flag SBD e allenamenti/settimana (lock input + chip), mostra banner informativo con link `Ricomincia da zero` e invia `cloneFromProgramId` nel body submit. `POST /api/programs` valida sorgente clone (404 not found, 403 ownership, 400 mismatch workoutsPerWeek) e copia `WorkoutSkeleton` in transazione atomica insieme alla creazione programma. Copertura test aggiunta con unit su schema/UI e integrazione clone branch (happy path, edge cases, rollback simulato).

### Fixed
- **[11 Maggio 2026] Fix completamento prematuro autosave serie trainee** — In alcuni casi la PATCH di autosave marcava un esercizio come completato anche con set mancanti, perche il calcolo usava solo le righe `sets_performed` presenti. Ora il completamento richiede tutte le set pianificate (`workout_exercise.sets`) presenti e completate, evitando toast anticipati di workout/settimana. Allineata anche la route di submit workout alla stessa regola di coerenza lato backend. **Files modificati:** `src/app/api/trainee/workout-exercises/[id]/feedback/route.ts`, `src/app/api/trainee/workouts/[id]/submit/route.ts`, `tests/integration/workout-exercise-feedback.test.ts`, `tests/integration/trainee-workout-submit.test.ts`.
- **[11 Maggio 2026] Mitigazione AbortError set-autosave trainee** — In `/trainee/workouts/[id]` poteva comparire toast tecnico `AbortError: Fetch is aborted` durante l'autosalvataggio della singola serie. Correzione: `keepaliveFetch` non applica piu un `AbortSignal.timeout` implicito (timeout ora solo opt-in con `timeoutMs`) e il catch di `persistExerciseFeedback` mappa gli `AbortError` su messaggio utente generico (`workouts.errorFeedback`) mantenendo tracciamento Sentry con tag `aborted`. **Files modificati:** `src/lib/keepalive-fetch.ts`, `src/app/trainee/workouts/[id]/_content.tsx`, `tests/unit/keepalive-fetch.test.ts`.
- **[11 Maggio 2026] Affidabilita autosave singola serie trainee** — Nella schermata `/trainee/workouts/[id]`, spuntando una serie con app in background o tab chiusa il record poteva non essere persistito e la riga sparire al ricaricamento. Risolto sulla path della singola serie: (1) `persistExerciseFeedback` usa fetch con `keepalive: true`, (2) `fetchWorkout` ora pad le `SetPerformed` fino a `we.sets` quando il feedback ha meno righe del prescritto, (3) il mount non rehydrata piu `feedbackData`/`exerciseRPE`/`exerciseNotes` da `localStorage` (server-first), eliminando shadowing dello stato server. La chiave `workout_<id>_feedback` continua a essere scritta in shape legacy per `ProgramDetailContent`. RPE handler, note handler, submit endpoint e schemi non sono stati toccati in questa iterazione. **Files modificati:** `src/app/trainee/workouts/[id]/_content.tsx`, `tests/unit/trainee-workout-focus.test.tsx`. **Files creati:** `src/lib/keepalive-fetch.ts`, `tests/unit/keepalive-fetch.test.ts`.
...
**File modificati:** `src/app/trainer/programs/[id]/edit/_content.tsx`, `implementation-docs/CHECKLIST.md`, `implementation-docs/CHANGELOG.md`
**Note:** La rimozione delle righe esercizio in edit programma trainer ora avviene in modo immediato al drop sul cestino: la riga viene nascosta subito in UI (optimistic update). Per le righe persistite parte subito la `DELETE` API (con `404` trattato in modo idempotente) e, in caso di errore, viene mostrato toast informativo con rollback della riga in vista. Le righe draft vengono rimosse localmente. Rimossa tutta la logica deferred basata su `pendingDeletesByWorkout` e relativo codice di cleanup nel salvataggio workout. Confermata inoltre la rimozione del delete in riga via conferma/modal: resta solo il flusso drag-and-drop sul cestino.

### [11 Maggio 2026] — Affidabilita autosave singola serie trainee

**Task checklist:** #11.99
**File modificati:** `src/app/trainee/workouts/[id]/_content.tsx`, `src/lib/keepalive-fetch.ts`, `tests/unit/trainee-workout-focus.test.tsx`, `tests/unit/keepalive-fetch.test.ts`, `implementation-docs/CHECKLIST.md`, `implementation-docs/CHANGELOG.md`
**Note:** Mitigazione mirata del bug "serie spuntata non persistita con app-switch": trasporto keepalive sul set-autosave, padding delle serie mancanti su reload e mount server-first senza rehydrate locale di `feedbackData`/`exerciseRPE`/`exerciseNotes`. Conservata la shape legacy del draft `workout_<id>_feedback` per compatibilita con `ProgramDetailContent`.

### [11 Maggio 2026] — Mitigazione AbortError set-autosave trainee

**Task checklist:** #11.100
**File modificati:** `src/lib/keepalive-fetch.ts`, `src/app/trainee/workouts/[id]/_content.tsx`, `tests/unit/keepalive-fetch.test.ts`, `implementation-docs/CHECKLIST.md`, `implementation-docs/CHANGELOG.md`
**Note:** Rimosso il timeout implicito dal wrapper keepalive per evitare abort client-side intermittenti; timeout disponibile solo in opt-in (`timeoutMs`). Nella UI workout trainee, gli errori di tipo `AbortError` non mostrano piu testo tecnico e vengono mappati al fallback utente `workouts.errorFeedback`, mantenendo telemetria Sentry con tag `aborted`.

### [11 Maggio 2026] — Fix completamento prematuro autosave serie trainee

**Task checklist:** #11.101
**File modificati:** `src/app/api/trainee/workout-exercises/[id]/feedback/route.ts`, `src/app/api/trainee/workouts/[id]/submit/route.ts`, `tests/integration/workout-exercise-feedback.test.ts`, `tests/integration/trainee-workout-submit.test.ts`, `implementation-docs/CHECKLIST.md`, `implementation-docs/CHANGELOG.md`
**Note:** Il completamento esercizio non viene piu calcolato sul solo sottoinsieme di righe `sets_performed` presenti. La logica usa il numero di set pianificate su `WorkoutExercise.sets`: `isCompleted=true` solo quando tutte le set pianificate sono presenti e `completed=true`. Questo evita la cascata anticipata su workout/settimana e i relativi toast mostrati troppo presto alla prima spunta.

### Added
- **[8 Maggio 2026] RPE Autosave Implementation** — New lightweight endpoint `PATCH /api/trainee/workout-exercises/[id]/feedback/rpe` saves only `actualRpe` via a dedicated route without touching sets, notes, or cascade logic. Client calls this endpoint directly when `RPESelector` fires `onChange`, updating local state optimistically and rolling back on error. Includes validation (5.0–10.0, 0.5 steps), ownership check, and error handling with i18n keys. **Files created:** `src/app/api/trainee/workout-exercises/[id]/feedback/rpe/route.ts`, `tests/unit/rpe-schema.test.ts`, `tests/integration/workout-exercise-rpe.test.ts`. **Files modified:** `src/schemas/feedback.ts`, `src/app/trainee/workouts/[id]/_content.tsx`. **Schema:** Added `workoutExerciseRpeSchema` and `WorkoutExerciseRpeInput` type. **Tests:** 5 unit + 6 integration tests, all passing.

### Added
- **[7 Maggio 2026] View programma usa pannelli compatti da pagina edit** — La pagina `/trainer/programs/[id]` mostrava `ReviewProgramContent` con una tabella semplice a 3 colonne. Sostituita con `EditProgramContent readOnly={true}` che mostra i pannelli compatti dell'editor (riscaldamento, esercizio, variante, serie, rip., RPE, peso, recupero) con settimane e workout collassabili, helper PR/SBD. Aggiunta gestione `searchParams` per `backHref` (stessa logica della pagina edit). **Files modificati:** `src/app/trainer/programs/[id]/page.tsx`.
- **[7 Maggio 2026] Fix: Salva bozza allo step skeleton chiama saveSkeleton()** — Il pulsante "Salva bozza" nello step 2 (skeleton) era un `<Link>` che navigava senza salvare. Sostituito con un `<button>` che chiama il nuovo handler `handleSaveSkeletonDraftAndBack()`: chiama `saveSkeleton()` (PUT `/api/programs/[id]/skeleton`) e solo in caso di successo fa `router.push('/trainer/programs')`. Mostra spinner durante il salvataggio e disabilita il pulsante se è in corso `applyingStructure`. **Files modificati:** `src/app/trainer/programs/[id]/edit/_content.tsx`.
- **[7 Maggio 2026] Skeleton Data Model Refactor (TRN-011) — completamento frontend** — Tasks 8-10: rimosso `isSkeletonExercise` da tutti i tipi e da `EditableWorkoutExerciseRow` in `_content.tsx` (7 occorrenze). Aggiunto campo `skeleton` all'interfaccia `Program`. Il GET di programma ora espone `skeleton` (rinominato da `workoutSkeletons`). Step 2 usa `program.skeleton` per idratare i `structureRowsByWorkoutIndex` al caricamento. Aggiunto callback `saveSkeleton` che chiama `PUT /api/programs/[id]/skeleton`; `applyStructureToAllWeeks` è ora `async` e chiama `saveSkeleton()` prima di modificare lo stato. Step 3 usa `hydrateDraftRowsForWorkout()` per creare le righe draft invece della logica inline. Aggiunte chiavi i18n `skeletonSaveError` in EN e IT. **Test:** `tests/integration/skeleton.test.ts` (8 test, tutti passing). **Files modificati:** `src/app/trainer/programs/[id]/edit/_content.tsx`, `src/app/api/programs/[id]/route.ts`, `public/locales/en/trainer.json`, `public/locales/it/trainer.json`, `implementation-docs/to-do-list-from-team.md`. **Files creati:** `tests/integration/skeleton.test.ts`.

### Added
- **[7 Maggio 2026] Skeleton Data Model Refactor (TRN-011)** — Moved wizard "skeleton" (step 2 structure rows) from `WorkoutExercise.isSkeletonExercise` flag into dedicated `WorkoutSkeleton` table. New endpoints: `PUT /api/programs/[id]/skeleton` for bulk replacement (draft-only), `GET /api/programs/[id]` now includes `skeleton` array. Step 2 persists across page refresh. Step 3 hydrates in-memory draft rows from skeleton for workouts with no saved exercises. Introduced helper utilities: `buildStructureRowsForWorkout()` in `structure-utils.ts`, `hydrateDraftRowsForWorkout()` in `skeleton-hydration.ts`. Migration: new `workout_skeletons` table (indexed by `(programId, dayIndex, order)`); dropped `isSkeletonExercise` column from `workout_exercises`. **Files created:** `src/schemas/skeleton.ts`, `src/app/api/programs/[id]/skeleton/route.ts`, `src/app/trainer/programs/[id]/edit/skeleton-hydration.ts`, `prisma/migrations/20260507000000_skeleton_table/migration.sql`. **Files modified:** `prisma/schema.prisma`, `src/app/api/programs/[id]/route.ts`, `src/app/api/programs/[id]/workouts/[workoutId]/exercises/{route,bulk}.ts`, `src/app/api/programs/[id]/copy-week/route.ts`, `src/lib/trainee-program-data.ts`, `src/app/trainer/programs/[id]/edit/structure-utils.ts`, `src/app/trainee/programs/_components/ProgramDetailContent.tsx`, `src/app/trainer/programs/[id]/workouts/[wId]/_content.tsx`.

### Added
- **[7 Maggio 2026] Drag-and-drop cestino in PersonalRecordsExplorer** — Rimosso il pulsante delete (`ActionIconButton variant="delete"`) dalle righe della tabella PR. Introdotto meccanismo drag-and-drop: righe draggabili quando `onDeleteRecord` è fornito. Zona cestino tra tabella e grafico, evidenziata in rosso durante il trascinamento. `showActions` ora dipende solo da `onEditRecord`. Token i18n `dragToDelete` / `dropToDelete` aggiunti in EN e IT. **File:** `src/components/PersonalRecordsExplorer.tsx`, `public/locales/{en,it}/common.json`.
